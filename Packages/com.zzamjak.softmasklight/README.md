...
알파 채널 기반 1-Pass 소프트 마스킹 UPM 패키지 (모바일 최적화) — `com.zzamjak.softmasklight`

## 설치

**OpenUPM (권장)** — Scoped Registry `https://package.openupm.com` / scope `com.zzamjak` 등록 후
...
**샘플**:
- `UIEffect Support` — com.coffee.ui-effect와 함께 쓸 때 임포트 (마스킹 오버라이드 셰이더)
- `Verify Scene` — 12개 마스킹 케이스 검증 씬

> 이전 이름: CAT SoftMask (사내 플러그인 v2.4.0에서 패키지 v1.0.0으로 분리 배포).
...
```
Packages/com.zzamjak.softmasklight/
├── Runtime/
...
│   ├── SoftMaskLight_TMP.shader          # TextMeshPro 전용 마스크 셰이더 (TMP 유니폼 자체 선언)
...
├── Samples~/
│   ├── UIEffectSupport/                  # UIEffect 마스킹 오버라이드 셰이더 (UIEffect 설치 시 임포트)
│   └── VerifyScene/                      # 12케이스 검증 씬
├── Resources/
│   └── SoftMaskLightSettings.asset       # Hidden 셰이더 빌드 포함 에셋 (사전 포함, 인스톨러가 갱신)
└── Editor/
    ├── CAT.SoftMaskLight.Editor.asmdef   # 에디터 어셈블리 (includePlatforms: Editor)
    ├── SoftMaskLightEditor.cs            # 커스텀 인스펙터
    ├── SoftMaskLightBuildProcessor.cs    # 빌드 직전 셰이더 등록 강제 갱신
    └── SoftMaskLightInstaller.cs         # 자동 설치/설정 관리자 (불변 패키지 대응)
```

> UIEffect 오버라이드 셰이더는 `Samples~`에 있어 기본적으로 임포트되지 않습니다.
> com.coffee.ui-effect가 없는 프로젝트에서 include 에러가 나지 않게 하기 위한 구조이며,
> UIEffect 사용 시 Package Manager에서 "UIEffect Support" 샘플을 임포트하면 됩니다.
